...
def compare_methods(csv1=None, csv2=None, method1=None, method2=None, csvs=None, output=None, latex_output=None):
    ...
    # Generate LaTeX table if requested (using existing create_latex_table function)
    if latex_output:
        dataframes = [df1, df2]
        labels = [name1 or "Method 1", name2 or "Method 2"]
        latex_code, n_samples = create_latex_table(dataframes, labels, common_metrics, baseline_idx=0)
...
def main():
    ...
    if args.csvs is not None and len(args.csvs) >= 2:
        ...
        if args.csv2 is None and args.method2 is None and args.method1 is not None:
            ...
            compare_methods(args.csv1, args.csv2, args.method1, args.method2, output=args.output, latex_output=args.latex)
    else:
