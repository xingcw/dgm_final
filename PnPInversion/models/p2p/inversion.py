...
class DirectInversion:
    ...
    def offset_calculate_with_controlnet(self, latents, num_inner_steps, epsilon, guidance_scale, 
                                          controlnet, controlnet_conditioning_image, controlnet_conditioning_scale):
        """
        Calculate offsets with ControlNet included in the denoising step.
        This ensures the offsets compensate for both CFG errors AND ControlNet influence,
        enabling perfect reconstruction when using ControlNet during the forward pass.
        """
        noise_loss_list = []
        latent_cur = torch.concat([latents[-1]]*(self.context.shape[0]//2))
        
        # Prepare control image
        batch_size = latent_cur.shape[0]
        if controlnet_conditioning_image.shape[0] != batch_size:
            control_image = controlnet_conditioning_image.repeat(batch_size, 1, 1, 1)
        else:
            control_image = controlnet_conditioning_image
        
        controlnet_dtype = next(controlnet.parameters()).dtype
        control_image_input = control_image.to(dtype=controlnet_dtype)
    ...
    def invert_with_controlnet(self, image_gt, prompt, guidance_scale, 
                                controlnet, controlnet_conditioning_image, controlnet_conditioning_scale,
                                num_inner_steps=10, early_stop_epsilon=1e-5):
        """
        Invert image with ControlNet-aware offset calculation.
        """
        self.init_prompt(prompt)
        register_attention_control(self.model, None)
        ...
        noise_loss_list = self.offset_calculate_with_controlnet(
    ...
    def invert_with_controlnet_separate_offsets(self, image_gt, prompt, guidance_scale, 
                                                 ...
                                                 num_inner_steps=10, early_stop_epsilon=1e-5):
        """
        Invert image and compute SEPARATE offsets for CFG and ControlNet.
        
        Returns:
            - noise_loss_full: Full offset (CFG + ControlNet) for source branch (d3)
            - noise_loss_cfg_only: CFG-only offset for target branch (d1)
            
        Rationale:
            - Source: d3 → perfect reconstruction
        ...
        # Compute CFG-only offset (d1) - for target branch
        noise_loss_cfg_only = self.offset_calculate(
            ddim_latents, num_inner_steps, early_stop_epsilon, guidance_scale
        )
        
        # Compute CFG + ControlNet offset (d3) - for source branch
        noise_loss_full = self.offset_calculate_with_controlnet(
            ddim_latents, num_inner_steps, early_stop_epsilon, guidance_scale,
            controlnet, controlnet_conditioning_image, controlnet_conditioning_scale
        )
